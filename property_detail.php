...
// Check if a specific property ID is passed
if (isset($_GET['property_id'])) {
    $property_id = $_GET['property_id'];

    // Fetch property details based on the property ID
    $sql_single = "
    SELECT properties.*, users.first_name, users.profile
    FROM properties
    JOIN sellers ON properties.seller_id = sellers.seller_id
    ...
    if (mysqli_num_rows($result_single) == 0) {
        echo "<h3>No property found with the given ID.</h3>";
        exit();
    }
} else {
        ...
        if (mysqli_num_rows($result_single) > 0) {
            while ($property = mysqli_fetch_assoc($result_single)) {
                ...
                echo "<div class='property-details'>
                        ...
                        <div class='icon-text'>
                            <i class='fas fa-user'></i><p>Seller Name: {$property['first_name']}</p>
                        </div>
                        <div class='icon-text'>
                            <img src='uploads/{$property['profile']}' alt='Seller Image' style='width: 200px; height: 200px; border-radius: 50%;'>
                        </div>
                        <a href='add_to_favorites.php?property_id={$property['property_id']}' class='btn btn-fav'><i class='fas fa-heart'></i> Add to Favorites</a>
                        <a href='schedule_meeting.php?property_id={$property['property_id']}&seller_id={$property['seller_id']}' class='btn btn-schedule'><i class='fas fa-calendar-alt'></i> Schedule Meeting</a>
                    </div>";
            }
        }
        ?>
